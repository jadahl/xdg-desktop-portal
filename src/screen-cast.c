...
static PipeWireRemote *
open_pipewire_screen_cast_remote (GVariant *results,
                                  GError **error)
{
  ...
  if (!g_variant_lookup (results, "streams", "a(ua{sv})", &streams_iter))
    {
      g_set_error (error, G_IO_ERROR, G_IO_ERROR_FAILED, "Missing PipeWire streams");
      return NULL;
    }
  ...
  n_streams = g_variant_iter_n_children (streams_iter);
      ...
      if (response == 0)
        {
          ...
          remote = open_pipewire_screen_cast_remote (results, &error);
          if (!remote)
            {
              g_warning ("Failed to open PipeWire remote: %s", error->message);
              g_clear_error (&error);
            ...
            {
              screen_cast_session->pipewire_remote = remote;
            }
        }
...
static gboolean
handle_take_pipewire_remote (XdpScreenCast *object,
                             GDBusMethodInvocation *invocation,
                             GUnixFDList *fd_list,
                             const char *arg_session_handle,
                             GVariant *arg_options)
{
  ...
  remote = screen_cast_session->pipewire_remote;
  if (!remote)
    {
      g_dbus_method_invocation_return_error (invocation,
                                             G_DBUS_ERROR,
  ...
  xdp_screen_cast_complete_take_pipewire_remote (object, invocation,
                                                 fd_list,
...
screen_cast_iface_init (XdpScreenCastIface *iface)
{
  ...
  iface->handle_take_pipewire_remote = handle_take_pipewire_remote;
}
...
screen_cast_session_close (Session *session)
{
  ...
  g_clear_pointer (&screen_cast_session->pipewire_remote,
                   (GDestroyNotify)pipewire_remote_destroy);
